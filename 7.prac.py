...
chart = alt.Chart(df_line_melted, title='호선별 시간대별 하차인원').mark_line().encode(
...
option = st.selectbox('몇 호선의 시간별 하차 인원수 데이터를 보여 드릴까요?', df_line_melted['호선'].unique())

# .loc 함수를 사용하여 선택한 호선 데이터 선별하고
# 새로운 데이터 프레임-에 저장 & 확인
df_selected_line = df_line_melted[df_line_melted['호선'] == option]
st.write(option, ' 데이터', df_selected_line)

# altair mark_area 차트 그리기
chart = alt.Chart(df_selected_line, title='선택 호선 시간대별 하차 인원').mark_area().encode(
...
option = st.selectbox('어떤 역의 시간별 하차 인원수 데이터를 보여 드릴까요?', df_off['역명'].unique())

# .loc 함수를 사용하여 선택한 역의 데이터를 선별하고
# 새로운 데이터 프레임-에 저장 & 확인
df_sta = df_off[df_off['역명']==option]

# 불필요한 컬럼 '연번','날짜','호선','역번호','역명','구분','합계' 제외하고 기존 데이터 프레임에 저장
df_sta = df_sta.drop(['연번','호선','역번호', '역명','구분','합계'], axis=1)
...
chart = alt.Chart(df_sta_melted, title='선택역 시간대별 하차 인원').mark_bar().encode(
x='시간', y='인원수').properties(width=650, height=350)

text = alt.Chart(df_sta_melted).mark_text(dx=0, dy=0, color='black').encode(
x='시간', y='인원수', text=alt.Text('인원수'))
